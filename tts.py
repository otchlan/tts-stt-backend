...
async def text_to_speech_from_file(file_path, language='pl'):
    try:
        ...
        if language not in VOICES:
            print(f"Unsupported language: {language}. Use 'pl' or 'en'.")
            return None
        ...
        for voice in VOICES[language]:
            try:
                ...
                print(f"Speech synthesis successful using {voice}.")
                print(f"Output saved to {output_filename}")
                return output_filename
            
            except Exception as voice_error:
                print(f"Failed with voice {voice}: {voice_error}")
        
        # If all voices fail
        print("Could not synthesize speech with any available voice.")
        return None
...
def main():
    ...
    try:
        ...
        if result:
            print(f"TTS conversion complete. File saved: {result}")
        else:
